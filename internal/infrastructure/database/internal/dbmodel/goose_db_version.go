...
func (o GooseDBVersionSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	for _, row := range o {
		...
		if len(wlCols) == len(gooseDBVersionAllColumns) {
			break
		}
	}
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to insert all from gooseDBVersion slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: failed to get rows affected by insertall for goose_db_version")
	}
