...
func (o *AssetType) GetTypeAssets() AssetSlice {
	if o == nil {
		return nil
	}

	return o.R.GetTypeAssets()
}
...
func (o AssetTypeSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	for _, row := range o {
		...
		if len(wlCols) == len(assetTypeAllColumns) {
			break
		}
	}
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: unable to insert all from assetType slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodel: failed to get rows affected by insertall for asset_types")
	}
...
// IMPORTANT: this will calculate the widest columns from all items in the slice, be careful if you want to use default column values
func (o AssetTypeSlice) InsertIgnoreAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
