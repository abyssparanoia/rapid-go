...
func (o StaffRoleSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	ln := int64(len(o))
	if ln == 0 {
		return nil
	}
	...
	for i, row := range o {
		...
		nzDefaults := queries.NonZeroDefaultSet(staffRoleColumnsWithDefault, row)
